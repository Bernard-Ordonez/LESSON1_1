...
{
    public partial class Lesson4_Example1 : Form
    {
        ...
        private void calculate_button_Click(object sender, EventArgs e)
        {
            try
            {
                amount_paid = Convert.ToDouble(amountPaid_textBox.Text);
                cash_given = Convert.ToDouble(cashGiven_textBox.Text);
                change = cash_given - amount_paid;
                change_textBox.Text = change.ToString("C");
                amountPaid_textBox.Text = amount_paid.ToString("C");
                cashGiven_textBox.Text = cash_given.ToString("c");
            }
            catch (Exception ex)
            {
